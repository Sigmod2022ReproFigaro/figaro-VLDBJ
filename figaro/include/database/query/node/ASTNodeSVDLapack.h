#ifndef _AST_NODE_SVD_LAPACK_H_
#define _AST_NODE_SVD_LAPACK_H_
...
namespace Figaro
{
    ...
    class ASTNodeSVDLapack: public ASTNode
    {
    ...
    public:
        ASTNodeSVDLapack(ASTNode *pOperand, const std::vector<std::string>& vRelationOrder, const std::vector<std::string>& vDropAttributes,
        uint32_t numThreads): m_pOperand(pOperand), m_vRelationOrder(vRelationOrder),
        m_vDropAttributes(vDropAttributes),
        m_numThreads(numThreads) {};
        virtual ~ASTNodeSVDLapack() override { delete m_pOperand; }
        ...
        virtual ASTNodeSVDLapack* copy() override
        {
            return new ASTNodeSVDLapack(m_pOperand->copy(),
