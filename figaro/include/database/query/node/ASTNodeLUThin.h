#ifndef _AST_NODE_LU_THIN_H_
#define _AST_NODE_LU_THIN_H_
...
namespace Figaro
{
    ...
    class ASTNodeLUThin: public ASTNode
    {
    ...
    public:
        ASTNodeLUThin(ASTNode *pOperand, const std::vector<std::string>& vRelationOrder, const std::vector<std::string>& vDropAttributes,
        uint32_t numThreads): m_pOperand(pOperand), m_vRelationOrder(vRelationOrder),
        m_vDropAttributes(vDropAttributes),
        m_numThreads(numThreads) {};
        virtual ~ASTNodeLUThin() override { delete m_pOperand; }
        ...
        virtual ASTNodeLUThin* copy() override
        {
            return new ASTNodeLUThin(m_pOperand->copy(),
