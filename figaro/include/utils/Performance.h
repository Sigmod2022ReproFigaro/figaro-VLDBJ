...
#define FIGARO_BENCH_INIT(timer)\
...
#define FIGARO_BENCH_START(timer) begin_##timer = std::chrono::high_resolution_clock::now();

#define FIGARO_BENCH_STOP(timer) finish_##timer = std::chrono::high_resolution_clock::now();\
elapsed_##timer = finish_##timer - begin_##timer;\
time_spent_##timer = elapsed_##timer.count();\
global_time_##timer += time_spent_##timer;

#define FIGARO_BENCH_GET_TIMER(timer)  std::to_string(global_time_##timer)
#define FIGARO_BENCH_GET_TIMER_LAP(timer)  std::to_string(time_spent_##timer)
